...
CUBE_COLOR = [("W","#ffffea"), ("R","#973aa8"), ("B","#01befe"), 
        ...
        while True:
            ...
            for ny, row in enumerate(level):  # Iterate over each row in the board ('level')
                ...
                for nx, cube in enumerate(row):  # Iterate over each cube in the row
                    if cube[0] == " " or cube[0] == "X": continue  # Ignore empty or blocked cubes
                    if cube[0] != "W" or cube[1] != "B":  # Check if cube's color matches the solution
                        solved = False
...
main()  # Likely restarts the game or level

# If there are no empty adjacent cells
...
#     # Restart the program after changing parameters
#     while True:
#         # Additional constants
#         WIN_WIDTH = SIZE_X * CUBE_SIZE  # Width of window
#         WIN_HEIGHT = SIZE_Y * CUBE_SIZE + PANEL_SIZE  # Height
#         DISPLAY = (WIN_WIDTH, WIN_HEIGHT)  # Grouping width and height into one variable
...
#         # Window initialization
#         screen = pygame.display.set_mode(DISPLAY)  # Create a window
#         pygame.display.set_caption("Rolling Cubes")  # Header
#         screen.fill(BACKGROUND_COLOR) # Bg color
...
#                 for ev in pygame.event.get():  # Processing events
#                     if (ev.type == QUIT) or (ev.type == KEYDOWN and ev.key == K_ESCAPE):
...
#                     #     mouse_x = button_reset_place.right
#                     #     mouse_y = button_reset_place.top
#             else:
#                 cube_pos_x = random.randint(0,SIZE_X-1)
#                 cube_pos_y = random.randint(0,SIZE_Y-1)
            ...
            #         # Determine the side that was pressed
            #         nn1 = xx2>yy2
            #         nn2 = (CUBE_SIZE-xx2)>yy2
            #         if nn1 and nn2:
            #             vek2 = 1
            ...
            #             # determine the number of empty cells around the cube (num)
            #             # if there is only one empty cell, then the direction of rolling is unambiguous.
            #             # if there are several, then you need to take into account the side of the cube to choose the direction
            #                 num = 0
            #                 if yy != 0:
            ...
            #         nyp = ny
                ...
                #     # find the position of the cube we are rolling
                #     fl = True
                #     if vek==1:
                #         if ny==SIZE_Y-1:
                #             fl = False
                ...
                #         nx -= 1
                # else:
                #     # play with the mouse, we already know the coordinates
                #     nxp = nx = cube_pos_x
                #     nyp = ny = cube_pos_y

                #     # find the position of an empty cell
                #     fl = True
                #     if vek==1:
                #         if nyp==0:
                #             fl = False
            ...
            # x = y = 0  # coordinates
            # for ny,row in enumerate(level):  # entire line
            #     for nx,cube in enumerate(row):  # each cube
            #         if cube[0]==" ":
            #             # empty
...
#                 num = 1
#                 for row in level_digit:  #  entire line
#                     if not solved: break
#                     for dig in row:  # each cube
#                         if dig == "0": continue
#                         if dig != str(num):
#                             solved = False
#                             break
...
#             pygame.display.update()  # update and display all changes on the screen.
# main()
